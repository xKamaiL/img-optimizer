...
WORKDIR ${GOPATH}/src/github.com/xkamail/img-optimizer
...
RUN go build -o ${GOPATH}/bin/img-optimizer ./main.go
...
COPY --from=builder /go/bin/img-optimizer /usr/local/bin/img-optimizer
...
ENTRYPOINT ["/usr/local/bin/img-optimizer"]
